...
// App State
let vouchers = JSON.parse(localStorage.getItem(STORAGE_KEY)) || SAMPLE_VOUCHERS;
...
// 2-FINGER PINCH ZOOM IN / PINCH ZOOM OUT FOR DOCUMENT PREVIEW
    ...
    tbody.innerHTML = `
      <tr>
        <td colspan="4" style="text-align: center; padding: 30px 10px; color: #94a3b8;">
          Belum ada dokumen bukti pembayaran.
        </td>
      </tr>
    `;
    return;
...
function initRecipientPortal(id) {
    ...
    if (container) {
      container.innerHTML = `
        <div class="text-center" style="padding: 40px 10px;">
          ...
          <h2 style="font-family: var(--font-serif); font-size: 18px;">Dokumen Tidak Ditemukan</h2>
          <p style="font-size: 12px; color: #64748b; margin-top: 4px;">Link bukti pembayaran ini tidak valid atau telah dihapus oleh Admin.</p>
        </div>
      `;
    }
  ...
  if (currentVoucher.status === 'Sudah Ditandatangani') {
    ...
    if (statusBadgeContainer) {
      statusBadgeContainer.innerHTML = `<span class="pill-badge pending"><i class="fa-solid fa-clock"></i> Menunggu TTD Anda</span>`;
    }
  ...
  if (btnConfirm) {
    btnConfirm.onclick = () => {
      ...
      if (currentVoucher && signatureDataUrl) {
        ...
        initRecipientPortal(currentVoucher.id);
      }
    };
  }
      ...
      <div class="ji-signatures-grid">
        <div class="ji-sig-box">
          ...
          <div class="ji-sig-space">
            <img src="ttd-diserahkan.png" alt="TTD Diserahkan Oleh" class="ji-sig-img">
          </div>
